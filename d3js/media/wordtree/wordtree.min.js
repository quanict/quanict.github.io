function resize() {
    width = vis.node().clientWidth, height = window.innerHeight - 50 - 0, heatmap.attr("transform", "translate(" + (width - 20.5) + ",.5)").select("rect.frame").attr("height", height - 1), svg.attr("width", width).attr("height", height), clip.attr("width", width - 30.5).attr("height", height), treeG.call(tree.width(width - 30).height(height - 20)), updateHeatmap(), text.call(textViewer)
}
...
function processText(e) {
    ...
    for (var i = []; t = re.exec(e);) {
        ...
        /\r\r|\r\r|/.test(l) ? (lines.push(i, []), i = [], o = t[0].length) : (o += t[0].length + !!l.length, o > c && (o = t[0].length, lines.push(i), i = []));
...
!function () {
    d3.longscroll = function () {
        function e(e) {
            function o(o) {
                e.each(function () {
                    this.scrollTop = o;
                    var e = d3.select(this), a = 1 + Math.ceil(this.clientHeight / i),
                        c = Math.max(0, Math.min(n - a, r)), l = c + a;
    ...
    function t() {
        ...
        var y, k, w, b, T = 5, C = 10, H = 15, A = .01, L = !1, M = s, _ = [], R = [], z = !1,
            E = d3.dispatch("prefix"), S = d3.scale.sqrt().range([0, 25]),
...
var re = new RegExp("[" + unicodePunctuationRe + "]|\\d+|[^\\d" + unicodePunctuationRe + "0000-001F007F-009F002000A01680180E2000-200A20282029202F205F3000".replace(/\w{4}/g, "\\u$&") + "]+", "g"),
    vis = d3.select("#vis"), svg = vis.append("svg"),
    ...
    heatmap = svg.append("g"), lines = [], text = d3.select("#text").on("scroll", scroll), hits = d3.select("#hits"),
    keyword = d3.select("#keyword"), source = d3.select("#source"), state = {}, tokens, selectedLines = [];
